...
//      const EventChannel('awareframework_Location/event');
  ...
  static const EventChannel _onLocationChangedStream =
      const EventChannel('awareframework_locations/event_on_location_changed');
  ...
  static const EventChannel _onExitRegionStream =
      const EventChannel('awareframework_locations/event_on_exit_region');
  ...
  static const EventChannel _onEnterRegionStream =
      const EventChannel('awareframework_locations/event_on_enter_region');
  ...
  Future<Null> start() {
    super
        .getBroadcastStream(_onLocationChangedStream, "on_location_changed")
        .map((dynamic event) =>
            LocationData.from(Map<String, dynamic>.from(event)))
        .listen((event) {
      this.data = event;
    ...
    super
        .getBroadcastStream(_onExitRegionStream, "on_exit_region")
        .map((dynamic event) =>
            GeofenceData.from(Map<String, dynamic>.from(event)))
        .listen((event) {
    ...
    super
        .getBroadcastStream(_onEnterRegionStream, "on_enter_region")
        .map((dynamic event) =>
            GeofenceData.from(Map<String, dynamic>.from(event)))
        .listen((event) {
...
class LocationData extends AwareData {
  ...
  LocationData() : this.from({});

  LocationData.from(Map<String, dynamic>? data) : super(data ?? {}) {
    if (data != null) {
...
class GeofenceData extends AwareData {
  ...
  GeofenceData() : this.from({});
  GeofenceData.from(Map<String, dynamic>? data) : super(data ?? {}) {
    if (data != null) {
      latitude = data["latitude"] ?? 0.0;
      longitude = data["longitude"] ?? 0.0;
      horizontalAccuracy = data["horizontalAccuracy"] ?? 0.0;
